...
void insererCompte(Compte *cpt)
{
  ...
  Compte *cptCour = tete, *cptPrec = NULL;

  // recherche position ou inserer le compte
  while (cptCour != NULL && !positionTrouvee)
  {
	  if (strcmp(cpt->nom, cptCour->nom) < 0)
      positionTrouvee = true;
    else
    {
      cptPrec = cptCour;
      cptCour = cptCour->suiv;
    }
  }
  // le compte doit etre insere entre cptPrec et cptCour
  if (cptPrec == NULL)
  {
    // insertion en tete (cptCour vaut tete)
    tete = cpt;
    cpt->suiv = cptCour;
  }
  else
  {
    cptPrec->suiv = cpt;
    cpt->suiv = cptCour;
    // si insertion en queue, cptCour vaut NULL, ca rentre dans le cas
    // ci-dessus
  }
}
